...
const dialogVariants = tv({
  slots: {
    ...
    content:
      "bg-background fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg p-6 shadow-lg duration-200 outline-none data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 sm:max-w-lg",
    close:
  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      >
        {children}
        {showCloseButton ? (
          <DialogPrimitive.Close className={close()} data-slot="dialog-close">
            <Icons.X />
...
const DialogHeader = ({ className, ...props }: ComponentProps<"div">) => {
  return <div className={header({ className })} data-slot="dialog-header" {...props} />;
};

const DialogFooter = ({ className, ...props }: ComponentProps<"div">) => {
  return <div className={footer({ className })} data-slot="dialog-footer" {...props} />;
};

const DialogTitle = ({ className, ...props }: ComponentProps<typeof DialogPrimitive.Title>) => {
  return (
    <DialogPrimitive.Title className={title({ className })} data-slot="dialog-title" {...props} />
  );
};
